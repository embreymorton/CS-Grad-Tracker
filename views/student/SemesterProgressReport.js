...
const main = (opts) => {
  ...
  return page(
    ...
    studentBar(opts),
    VA.allow("admin student advisor", ["_id", form.employmentAdvisor?._id])
      ? mainContent(opts)
...
const mainContent = (opts) => {
  ...
  return renderCard("Semester Progress", [
    h4(lastName, ", ", firstName),
    h3("Semester Progress Report"),
    a(
      { href: seeAllSubmissions, class: "btn btn-primary" },
    ...
    div(
      { class: "text-left" },
      div(
        "Please discuss your semester goals and milestones with your advisor. Fill out this form with as much details as you can. Below is a recommended/typical timeline with some major milestones for your reference:"
      ),
      details(
        { open: true },
        summary(h5({ class: "d-inline" }, strong("Recommended Milestones:"))),
        div(
          milestoneBullets("By the end of semester 4", [
            "Gain admission to PhD candidacy through PRP and faculty vote.",
          ]),
          milestoneBullets("By the end of semester 5", [
            "Discuss research plan with at least three (potential) committee members; submit form CS-12.",
          ]),
          milestoneBullets("By the end of semester 6", [
          ...
          milestoneBullets("By the end of semester 7", [
            "Submit a dissertation proposal to the committee; hold meeting for approval of proposal,",
            "OR Pass the Doctoral Oral examination.",
          ]),
          milestoneBullets("By the end of semester 8", [
            "Submit disseration proposal AND pass the Doctoral Oral examination.",
            "Apply for Admission to Candidacy for a Doctoral Degree (Graduate School form).",
          ]),
          milestoneBullets("At any time", [
            "Satisfy the program product requirement: submit Form CS-13.",
            "Satisfy the teach requirement: submit Form CS-11",
          ...
          milestoneBullets("By the end of semester 10", [
            "When dissertation is in substantially finished form, announce dissertation defense, giving two weeks's notice.",
...
const milestoneBullets = (title, listOfBullets) => {
  ...
  return [strong(title), ul(listOfBullets.map((bulletText) => li(bulletText)))];
};
...
const progressReportForm = (opts) => {
  ...
  const employmentAdvisorChoices = activeFaculty.map((faculty) =>
    makeOption(
  ...
  if (student.advisor) {
    // default option that is identical to selecting their normal advisor
    employmentAdvisorChoices.unshift(
      makeOption(
        student.advisor?._id.toString(),
        "I am employed by my advisor.",
        form.employmentAdvisor == undefined,
        true
      )
    );
  }
  const OTHER_TEXT = "Other, please specify:";
  employmentAdvisorChoices.push(
    makeOption(
      // other option for employment outside department
      student.advisor?._id.toString(),
      OTHER_TEXT,
  ...
  return x("form.cs-form#cs-form")(
    ...
    frow(
      div(
        "Q1: What progress did you make this semester (or since your last progress report)? Please include any milestones completed, papers submitted, or other general progress made."
      ),
      "progressMade",
      8,
      isStudent
    ...
    frow(
      div(
        "Q2: What are your goals/plans for next semester (including summers)?"
      ),
      "goals",
      8,
      isStudent
    ...
    vert,
    frow(
      div("Q3: List your prefrences for RA/TA positions next semester:"),
      "rataPreferences",
      8,
      isStudent
    ),
    ...
    row(
      colMd(6)(
        dropdown("employmentAdvisor", employmentAdvisorChoices, {
          isDisabled: isComplete || VA.not("admin student"),
          blankOption:
            "You do not have an advisor specified and so you must select from the dropdown.", // overriden above if student has an advisor
          isRequired: true,
        })
      ),
      colMd(6)(
        input("altEmploymentAdvisor", form.altEmploymentAdvisor, {
          isDisabled: isComplete,
          isRequired: true,
          isHidden: !form.altEmploymentAdvisor,
        })
      ),
      script(
        opts.cspNonce,
...
const formRow =
  (values, editAccess, type = "text") =>
  (label, name, width = 6, required = true) => {
    const element = input(name, values[name], {
      isRequired: required,
      isDisabled: !(editAccess && !complete),
    });
    return row(colMd(width)(label, element));
  };
...
const textareaRow =
  (form, editAccess) =>
  (label, name, width, required = true) => {
    const isDisabled = !(editAccess && !complete);
    const element = textarea(name, form[name], {
      rows: width,
      isRequired: required,
      isDisabled,
    });
...
const evaluationSection = (opts) => {
  ...
  return [
    ...
    div(
      ...
      div("Faculty members,"),
      div(
        "Please fill out this form for each student you are supervising. The recommended milestones of a graduate students are described above."
      )
    ),
    hr(),
      ...
      dropdown(
        ...
        optionSet(
          [
            ...
            [
              2,
              "2: WEAK: The student made lower than expected progress, but the student and I have made a plan to remedy it.",
            ],
            [
              1,
              "1: POOR: The student has not been making sufficient progress, and should be put on probation.",
            ],
          ],
    ...
    rowCol(
      12,
      b(
        form.altEmploymentAdvisor
          ? `⚠️ NOTE: You, ${student.advisor.fullName}, must complete this section for ${form.altEmploymentAdvisor} as well.`
          : ""
      )
    ),
    rowCol(
      ...
      dropdown(
        ...
        ),
        {
          isRequired: false,
          isDisabled:
            VA.not("admin", ["_id", form.employmentAdvisor?._id]) &&
            !form.altEmploymentAdvisor,
        }
      )
    ),
    vert,
    textFrow(
      VA.allow("admin", ["_id", form.employmentAdvisor?._id]) ||
        form.altEmploymentAdvisor
    )(
    ...
    viewerApprovalRow(
      ...
      cspNonce,
      {
        signerTitle: "Employment Advisor",
        isDisabled: VA.not("admin", ["_id", form.employmentAdvisor?._id]),
      }
    ),
    script(
      cspNonce, // hacky way to add "on behalf of"
      `
