...
const main = (opts) => {
  ...
  return page(
    ...
    studentBar(opts),
    VA.allow("student advisor", ["fullName", form.instructorSignature])
      ? mainContent(opts)
...
const mainContent = (opts) => {
  ...
  const form = !hasAccess ? div("You do not have access") : cs02Form(opts);
  return [
...
const cs02Form = (opts) => {
  ...
  return renderCard(
    ...
    x("form.cs-form#cs-form")(
      ...
      namePidDateRow(opts, editAccess),
      hr(),
      frow(div("Course Number:"), "courseNumber"),
      vert,
      frow(basisForWaiverLabel, "basisWaiver"),
      hr(),
      ...
      signatureDropdown(
        ...
        cspNonce,
        {
          ...
          checkboxAccess:
            (VA.isAdvisor &&
              activeFaculty.every(
                (f) => f.fullName != form.instructorSignature
              )) ||
            VA.allow("admin", ["fullName", form.instructorSignature]),
          allowOther: true,
          viewer,
        }
      ),
...
const formRow = (values, editAccess) => (label, name) => {
  ...
  return row(colMd(6)(label, value));
};
