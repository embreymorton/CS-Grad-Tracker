...
const main = (opts) => {
  ...
  return page(
    ...
    VA.allow(
      ...
      ["fullName", form.primarySignature],
      ["fullName", form.secondarySignature]
    )
      ? mainContent(opts)
...
const mainContent = (opts) => {
  ...
  return [
    ...
    hr(),
    div({ class: "text-left" }, cs08Form(opts)),
  ];
};
...
const cs08Form = (opts) => {
  ...
  return renderCard(
    ...
    x("form.cs-form#cs-form")(
      ...
      div(
        "The final draft to the primary reader must be delivered 4 weeks before the last day of classes of the semester in which the student plans to graduate.  We recommend that you work with the primary reader throughout the semester, first creating an outline, and then iterating over several drafts."
      ),

      readerDateRow(opts, editAccess, "primary"),
      vert,
      div(
        "The final draft to the secondary reader must be delivered 2 weeks before the last day of classes of the semester in which the student plans to graduate."
      ),
      readerDateRow(opts, editAccess, "secondary"),
      hr(),

      p(
        "Once the paper has been approved by both readers, the student is to upload it to the Carolina Digital Repository. Instructions for uploading:"
      ),
      ol(
        li(
          "Go to Carolina Digital Repository (cdr.lib.unc.edu) and login with your onyen."
        ),
        li("Select Student Papers"),
        li("Select Master's Papers and then “Create Work”"),
        li("Select “Department of Computer Science”"),
        li(
          "Fill in the “Work Deposit Form.” You do not need an ORCID. Check the deposit agreement and save."
        ),
      ...
      div("Attachment Link"),
      bcInput("attachmentURL", form.attachmentURL, {
      ...
      strong("Approval"),
      div(
        "We have judged this paper in both substance and presentation to satisfy the writing requirement for the M.S. in Computer Science."
      ),
...
const readerDateRow = (opts, editAccess, modifier) => {
  ...
  return row(
    isComplete
      ? colMd(6)(div(`${modifierLabel} Reader`), pseudoInput(readerValue))
      : colMd(6)(
          div(`${modifierLabel} Reader`),
          facultyDropdown(readerField, readerValue, activeFaculty, {
...
const approvalRow = (opts, modifier) => {
  ...
  const dateSignedMMDDYYYY = `${
    dateSigned.getMonth() + 1
  }/${dateSigned.getDate()}/${dateSigned.getFullYear()}`;
  ...
  const approvalLabel = isApproved
    ...
    : `(Not yet approved.)`;
  return [
    row(colMd(6)(pseudoInput(readerName, `#${modifier}-pseudo`))),
    vert,
    row(
      !isStudent
        ? colMd(6)(
            x(`input#${dateField}Checkbox.pseudo-checkbox`)({
              type: "checkbox",
              checked: isApproved ? "checked" : undefined,
              disabled: VA.not("admin", ["fullName", readerName]) || undefined,
            }),
            x(`input#${dateField}`)({
              type: "hidden",
              name: dateField,
              value: isApproved ? dateSigned.toString() : undefined,
            }),
            x(`input#${sigField}`)({
              type: "hidden",
              name: sigField,
              value: form[sigField] ? form[sigField] : "",
            }),
    ...
    row(colMd(5)(x(`em#${dateField}Label`)(approvalLabel))),
  ];
};
