...
const main = (opts) => {
  ...
  return page(
    ...
    studentBar(opts),
    VA.allow("student advisor", ["fullName", form.chairman])
      ? mainContent(opts)
...
const mainContent = (opts) => {
  ...
  return [
    ...
    h3("CS-06"),
    div({ class: "text-left" }, cs06Form(opts)),
  ];
};
...
const cs06Form = (opts) => {
  ...
  const {
    dissTitle,
    comp915,
    breadthCourseCategory,
    breadthCourseInfo,
    breadthCourseDate,
    breadthCourseGrade,
    concentrationCourseInfo,
    concentrationCourseDate,
    concentrationCourseHours,
    otherCourseInfo,
    otherCourseHours,
    note,
    otherCourses,
    minor,
    backgroundPrepWorkSheet,
  ...
  return renderCard(
    ...
    x("form.cs-form#cs-form")(
      ...
      div(
        "Note:  For Courses taken other than at UNC-CH, attach at least a catalog description and preferably a syllabus. "
      ),
      vert,
        ...
        colMd(2)(
          isComplete
            ...
            : select(
                ...
                option(
                  { value: "taken", selected: comp915 == "taken" || null },
                  "Taken"
                ),
                option(
                  { value: "waived", selected: comp915 == "waived" || null },
                  "Waived"
                )
              )
        )
      ...
      div(
        "List the 6 courses taken to satisfy the distribution requirement.  Include the following information:"
      ),
      div(
        a(
          {
      ...
      row(
        colMd(2)(
          ...
          range6.map((i) =>
            isComplete
              ? pseudoInput(breadthCourseCategory && breadthCourseCategory[i])
              : select(
                  {
                    name: "breadthCourseCategory",
                    required: true,
                    ...disabled,
                  },
                  option({ value: "" }, ""),
          ...
          range6.map((i) =>
            editAccess && !isComplete
              ? input(
                  "text",
                  "breadthCourseInfo",
                  breadthCourseInfo && breadthCourseInfo[i],
                  true
                )
        ...
        colMd(2)(
          ...
          range6.map((i) =>
            semesterInput(
              "breadthCourseDate",
              breadthCourseDate && breadthCourseDate[i],
              {
        ...
        colMd(2)(
          ...
          range6.map((i) =>
            gradeDropdown(
              "breadthCourseGrade",
              breadthCourseGrade && breadthCourseGrade[i],
              !editAccess || isComplete,
      ...
      row(
          ...
          range4.map((i) =>
            editAccess && !isComplete
              ? input(
                  "text",
                  "concentrationCourseInfo",
                  concentrationCourseInfo && concentrationCourseInfo[i]
                )
              : pseudoInput(
                  concentrationCourseInfo && concentrationCourseInfo[i]
                )
        ...
        colMd(3)(
          ...
          range4.map((i) =>
            semesterInput(
              "concentrationCourseDate",
              concentrationCourseDate && concentrationCourseDate[i],
              {
                isDisabled: !editAccess || isComplete,
                isSS_YYYY: false,
                placeholder: "SS YYYY",
          ...
          range4.map((i) =>
            editAccess && !isComplete
              ? input(
                  "number",
                  "concentrationCourseHours",
                  concentrationCourseHours && concentrationCourseHours[i]
                )
              : pseudoInput(
                  concentrationCourseHours && concentrationCourseHours[i]
                )
      ...
      div(
        "Indicate department, course number, title, and credit hours.  Add note if not taken at UNC-CH. "
      ),
      row(
        colMd(4)(
          ...
          range4.map((i) =>
            editAccess && !isComplete
              ? input(
                  "text",
                  "otherCourseInfo",
                  otherCourseInfo && otherCourseInfo[i]
                )
          ...
          range4.map((i) =>
            editAccess && !isComplete
              ? input(
                  "number",
                  "otherCourseHours",
                  otherCourseHours && otherCourseHours[i]
                )
      ...
      row(
        colMd(4)(
          div(
            "List any other courses here that you believe are relevant to your program of study."
          ),
          isComplete
            ? pseudoInput(otherCourses)
            : x("textarea.form-control")(
      ...
      row(
        colMd(4)(
          div(
            "See policy in ",
            a(
              { href: "https://handbook.unc.edu/phd.html" },
      ...
      div(
        "(it is not necessary to have all other requirements completed to turn in your program of study)"
      ),
      div("A. Background Preparation worksheet*"),
      colMd(4)(
        isComplete
          ? pseudoInput(backgroundPrepWorkSheet)
          : select(
              { name: "backgroundPrepWorkSheet", ...disabled },
              option({ value: "" }, ""),
              option(
                { value: "false", selected: !backgroundPrepWorkSheet || null },
                "false"
              ),
              option(
                { value: "true", selected: backgroundPrepWorkSheet || null },
                "true"
              )
            )
      ),
      ...
      colMd(4)(
        isComplete
          ? pseudoInput(programProductRequirement)
          : select(
              ...
              option(
                {
                  value: "false",
                  selected: !programProductRequirement || null,
                },
                "false"
              ),
              option(
                { value: "true", selected: programProductRequirement || null },
                "true"
              )
            )
      ),
      ...
      colMd(4)(
        isComplete
          ? pseudoInput(PHDWrittenExam)
          : select(
              { name: "PHDWrittenExam", ...disabled },
              option({ value: "" }, ""),
              option(
                { value: "false", selected: !PHDWrittenExam || null },
                "false"
              ),
              option(
                { value: "true", selected: PHDWrittenExam || null },
                "true"
              )
            )
      ),
      ...
      colMd(4)(
        isComplete
          ? pseudoInput(PHDOralExam)
          : select(
              { name: "PHDOralExam", ...disabled },
              option({ value: "" }, ""),
              option(
                { value: "false", selected: !PHDOralExam || null },
                "false"
              ),
      ...
      div(
        "Each committee member should review the program of study.  The Chair is requested to check off or initial beside each member who has reviewed this program of study. "
      ),
      ...
      row(
        colMd(6)(
          x("div.form-group.row")(
            x("label.col-md-2")(`Chair: `),
            colMd(10)(
              facultyDropdown("chairman", chairman, faculty, {
                isDisabled: isComplete || !editAccess,
                isRequired: true,
                blankOption: "Select chair member.",
              })
            )
          ),
          range7.map((i) =>
            x("div.form-group.row")(
              ...
              colMd(10)(
                editAccess && !isComplete
                  ...
                  : x("div.committee-name")(
                      pseudoInput(committee && committee[i])
                    )
              )
            )
          )
      ...
      div(
        strong("Committee advisor"),
        " (or Chair if different from advisor)*"
      ),
      div(
        "By signing here, the chair indicates that all items above have been approved by the majority of the committee."
      ),
      signatureRow(
        VA.allow("admin", ["fullName", chairman]),
        "chair",
        form,
        opts.cspNonce
      ),
      script(
        opts.cspNonce, // hacky way to add chair's name to label, relies on order of event listener attachment
        `
      ...
      div(
        ...
        row(
          colMd(6)(
            textarea("approvalReason", form.approvalReason, {
              isDisabled: isStudent || !admin,
      ...
      hr(),
      row(colMd(5)(div("Director of Graduate Studies Approval:*"))),
      adminApprovalCheckboxRow(viewer, "director", form, opts.cspNonce),
      vert,

      buttonBarWrapper(
        !isComplete ? [vert, x("div.hidden")({ id: "errorMessage" })] : null,
        submitButton(opts),
...
const namePidRow = (opts, editAccess) => {
  ...
  return row(
    colMd(4)(div("Name"), pseudoInput(name)),
    colMd(4)(div("PID"), pseudoInput(pid)),
    colMd(4)(
